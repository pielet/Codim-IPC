    ...
    T& value)
{
    ...
    if (withPenaltyCollision) {
        Compute_Plane_Collision_Potential<T, dim>(plane, X, Xn, h, value);
    }
    ...
    GROUND<T, dim>& plane)
{
    ...
    if (withPenaltyCollision) {
        Compute_Plane_Collision_Gradient<T, dim>(plane, X, Xn, h, nodeAttr);
    }
}
    ...
    bool projectDBC=true)
{
    ...
    if (withPenaltyCollision) {
        Compute_Plane_Collision_Hessian<T, dim>(plane, X, Xn, h, triplets);
    }
    if (DBCStiff) {
