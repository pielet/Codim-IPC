...
public: // API
        ...
        T voxelSize)
    {
        ...
        Eigen::Array<long, dim, 1> vcl = (range * one_div_voxelSize).ceil().template cast<long>().max(Eigen::Array<long, dim, 1>::Ones());
    ...
    void Query_Point_For_Points(const Eigen::Matrix<T, dim, 1>& pos,
        T radius, std::unordered_set<int>& pInds) const
    {
        Eigen::Array<int, dim, 1> mins, maxs;
        Locate_Voxel_Axis_Index(pos.array() - radius, mins);
        Locate_Voxel_Axis_Index(pos.array() + radius, maxs);
    ...
    void Query_Triangle_For_Edges(
        const Eigen::Matrix<T, dim, 1>& pos0,
        const Eigen::Matrix<T, dim, 1>& pos1,
        const Eigen::Matrix<T, dim, 1>& pos2,
        T radius, std::unordered_set<int>& eInds) const
    {
        Eigen::Array<int, dim, 1> mins, maxs;
        ...
        T& curMaxStepSize, T voxelSize, T thickness)
    {
        ...
        threadsP.Par_Each([&](int svI, auto data) {
            int vI = boundaryNode[svI];
            Eigen::Matrix<T, dim, 1> minCoord = SV.row(svI).array().min(SVt.row(svI).array()) - thickness / 2;
            Eigen::Matrix<T, dim, 1> maxCoord = SV.row(svI).array().max(SVt.row(svI).array()) + thickness / 2;
    ...
    void Query_Point_For_Points(int svI, std::unordered_set<int>& vInds) const
    {
        vInds.clear();
        for (const auto& voxelInd : pointAndEdgeOccupancy[svI]) {
            const auto& voxelI = voxel.find(voxelInd);
